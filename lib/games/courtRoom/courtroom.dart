...
class _CourtroomGameScreenState extends State<CourtroomGameScreen> with SingleTickerProviderStateMixin {
  ...
  void _startTimer() {
    ...
    _countdownTimer = Timer.periodic(Duration(seconds: 1), (timer) {
      if (_timer > 0 && _showQuestion) {
        ...
        _countdownTimer.cancel();
        _answerQuestion(''); // Automatically mark as wrong when time runs out
      }
    });
  }
  ...
  void _answerQuestion(String answer) {
    ...
    if (isCorrect) {
      // Update score
      setState(() {
        _score += 1; // Increment score by 1 on correct answer
      });
    }

    // Play sound based on answer correctness
    if (isCorrect) {
      _audioPlayer.play('assets/sounds/correct.mp3' as Source);
    } else {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
            isCorrect
                ? 'Well done! You got the right answer. +1 Point.'
                : 'Oops! The correct answer was "$_correctAnswer".'),
        actions: [
          TextButton(
            onPressed: () {
  ...
  void _nextPart() {
    if (_storyIndex < _storyParts.length - 1) {
      ...
      _startTimer(); // Start the timer when the question is shown
    }
  }
  ...
  void _restartGame() {
    setState(() {
      ...
      _timer = 30; // Reset timer to 30 seconds
    });
    _startTimer(); // Restart timer after game restart
  }
